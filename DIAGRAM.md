...
sequenceDiagram
    ...
    rect rgb(30, 40, 30)
        Note over Pop: Hybrid Asymmetric Encryption (Zero-Knowledge)
        Pop->>Pop: Import Site's Public Key (RSA-OAEP)
        Pop->>Pop: Generate random AES-256 Data Key
        Pop->>Pop: Encrypt Data with AES Key (AES-GCM)
        Pop->>Pop: Wrap AES Key with Site's Public Key
        Pop->>Pop: Format: base64(WrappedKey) . base64(IV) . base64(Data)
    end
    
    Pop->>S: POST /api/generate-jwt (Hybrid Payload)
    S->>S: Sign with RS256 Private Key
    S-->>Pop: signed_token (JWT with "encrypted" claim)
    
    Pop->>Site: postMessage({ type: 'OXID_AUTH_SUCCESS', token })
    Pop->>Pop: window.close()
    
    Site->>Site: Decrypt with Site's Private Key:
    Site->>Site: 1. Unwrap AES Key (RSA-OAEP)
    Site->>Site: 2. Decrypt Payload (AES-GCM)
```
...
| **Hybrid Encryption** | RSA-OAEP 2048 + AES-GCM 256. | Allows encrypting large profile payloads that exceed RSA direct encryption limits, while maintaining Asymmetric security. |
| **Per-site Isolation** | Site-specific RSA Keypair. | Ensures data for one site cannot be decrypted by another site. Keys are generated by Oxidiko but the Private Key is only known by the site owner. |
